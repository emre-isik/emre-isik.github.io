...
Mean surface magnetic field strength vs. rotation period, after correcting G-star measurements for metallicity and effective temperature. Coloured diamonds and dashed curves show FEAT model predictions for different flux-injection scaling exponents <em>p</em> and <em>q</em>. Orange symbols show K5–M0 stars (Reiners+22), included for reference without stellar-parameter correction. The Sun is shown as an open circle at 25 d.
</p>

How steeply does the rate of magnetic flux emergence grow as a solar-type star spins faster? Emerging flux is not directly observable on other stars, so this has remained poorly constrained. **This study provides the first semi-empirical constraints on the magnetic flux injection rate at the surface of solar-type stars as a function of rotation, offering a direct benchmark for stellar dynamo models.** Using the FEAT model, we tested a family of power-law scalings for the emerging flux and compared the resulting surface-averaged unsigned field strengths with direct Zeeman measurements of G stars. The emergence rate must scale steeply with rotation (power-law exponent ≈ 1.9), significantly steeper than the linear scaling assumed previously. For the most rapidly rotating stars (~3-day periods), active-region-driven fields contribute up to 82% of the total surface flux; for the Sun, more than 90% of the disc-integrated Zeeman signal comes from small-scale dynamo (SSD) fields generated by near-surface magnetoconvection, unrelated to the global dynamo.

A complication is that metallicity and temperature introduce significant scatter in the observed rotation-magnetism relation: model residuals correlate strongly with metallicity (_r_ = 0.83) and effective temperature (_r_ = −0.76), combined _r_ = 0.90. More metal-rich stars show systematically stronger surface fields at a given rotation rate, likely because higher metallicity deepens the convection zone -- analogous to having a cooler star. After correcting for both parameters (figure above), the data align closely with the steep model curves. Controlling for stellar parameters is essential for unbiased rotation-magnetism relationships, regardless of measurement technique.
...
Mapping the distribution of starspots across a star's surface is useful for understanding stellar magnetic fields and their connection to flare activity. Traditional Doppler imaging analyses how starspots distort spectral line profiles as the star rotates, but struggles to recover spots at low latitudes and in the less-visible hemisphere. Light-curve modeling tracks brightness variations as spots rotate in and out of view, but cannot uniquely determine spot latitudes. We present the first study combining Doppler imaging with simultaneous high-cadence space photometry and systematic injection-recovery tests. We revisit the rapidly rotating K2-type star PW Andromedae. Using high-resolution GAOES-RV spectra from the 3.8 m Seimei telescope alongside continuous TESS photometry, we updated [**SpotDIPy**](https://github.com/EnginBahar/SpotDIPy) (Bahar et al. 2024, see below, with PW And as the test case) so that it now supports joint spectroscopic and photometric inversions. The combined approach reveals significant spot features at mid-to-low latitudes and even in the southern hemisphere (**left panel**), where spectroscopy-only reconstructions fail due to weak line-profile signatures (**right panel**). Our injection-recovery tests confirm that adding high-cadence photometry dramatically improves latitude recovery and spot area estimates, especially under realistic conditions with incomplete rotational phase coverage and moderate signal-to-noise ratios. The reconstruction shows ~10% of the visible surface is covered by cool spots. By comparing the spot map with TESS flare detections, we find potential associations between flare timing and activity patterns, demonstrating how improved surface mapping helps localise magnetic energy release events on active stars.
